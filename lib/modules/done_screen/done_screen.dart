...
class DoneScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<CounterCubit,CounterStates>(
      ...
      builder: (context,state)=> ListView.separated(itemBuilder:(context,index)=>tasks(model: CounterCubit.get(context).doneTasks[index],context: context),
          separatorBuilder: (context,index)=>Container(
            width: double.infinity,
            height: 1,
            color: Colors.black,

          ), itemCount: CounterCubit.get(context).doneTasks.length),
